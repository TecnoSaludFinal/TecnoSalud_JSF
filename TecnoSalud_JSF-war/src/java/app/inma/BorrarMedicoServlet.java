     ...
     // Hago una lista de las citas que este medico tiene   
        ...
        if (listaCitas == null)
        {
            //Si la lista de citas está vacía, significa que el médico no tiene citas, con lo que podemos eliminarlo
                ...
                facadeMedico.remove(medico);

                List<Medicos> lista = (List<Medicos>) facadeMedico.findAll();
                for(int i=0;i<lista.size();i++)
                {
                    Medicos Modpc = lista.get(i); 
                    facadeMedico.actualizar(Modpc, i);
                }
        ...
        else
        {
            ...
            List <Medicos> m = (List <Medicos>)facadeMedico.findByIdAllEspecialidad(medico.getIdEspecialidad().getNombre());
            ...
            for(int i=0;i<listaCitas.size();i++)
            {
                ...
                if(ale != medico.getIdMedicos())
                {
                    ...
                    for(int j=0;j<m.size();j++)
                    {
                        ...
                        mm = (Medicos) facadeMedico.findById(ale);
                        c.setIdMedicos(mm);
                        facadeCitas.edit(c);
                    }
                }  
